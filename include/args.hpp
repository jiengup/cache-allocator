                   ...
                   CacheAllocation::allocation_arg_t *sargs)
{
  ...
  try
  {
    ...
    options
      ...
      ("f,input-file-path", "trace file path/dp table file path", cxxopts::value<std::string>(sargs->input_file_path))
    ...
    std::vector<std::string> path_token = mysplit(sargs->input_file_path, "/");
