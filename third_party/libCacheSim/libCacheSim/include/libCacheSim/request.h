...
#else
  printf("req real_time %lu, id %llu, ori_id %s, size %ld, tenant %d, op %s, valid %d\n",
         (unsigned long)req->clock_time, (unsigned long long)req->obj_id, req->origin_key,
         (long)req->obj_size, req->tenant_id, req_op_str[req->op], req->valid);
#endif
}
