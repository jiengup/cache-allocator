...
static inline void csv_cb1(void *s, size_t len, void *data) {
  ...
  if (csv_params->curr_field_idx == csv_params->obj_id_field_idx) {
    if (reader->obj_id_is_num) {
      ...
      if (csv_params->use_origin_key) {
        memcpy(req->origin_key, (char *)s, len);
      }
    }
  ...
  } else if (csv_params->curr_field_idx == csv_params->tenant_id_field_idx) {
    req->tenant_id = (uint32_t)strtoul((char *)s, &end, 0);
    if (req->tenant_id == 0 && end == s) {
      ERROR("csvReader tenant_id is not a number: \"%s\"\n", (char *)s);
    }
  }
...
void csv_setup_reader(reader_t *const reader) {
  ...
  csv_params->tenant_id_field_idx = init_params->tenant_id_field;
  csv_params->use_origin_key = init_params->use_origin_key;
